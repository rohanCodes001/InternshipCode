...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  8 16:06:57 2022
...
num = 15   # This number controls which well concentration time series data to use
...
df = ssa.components_to_df(n = 19)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W12-3-Concentration.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 19) and (19-40).
ssa.calc_wcorr()
ssa.plot_wcorr()
...
# %%
# Looking at First 19 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,19)).plot()
ssa.reconstruct(slice(19,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 19 Components", "Remaining Components", "Original TS"]);
...
df = ssa.components_to_df(n = 34)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W12-3-CTMass.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 34) and (34-40).
ssa.plot_wcorr()
...
# %% Looking at First 34 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,34)).plot()
ssa.reconstruct(slice(34,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 34 Components", "Remaining Components", "Original TS"]);
plt.title("Original Time Series versus Reconstructed Time series.")
plt.xlabel("Number of Records in Time")
...
df = ssa.components_to_df(n = 35)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W12-3-CTAqMass.joblib', compress=4)
...
# %% Looking at First 35 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,35)).plot()
ssa.reconstruct(slice(35,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 35 Components", "Remaining Components", "Original TS"]);
plt.title("Original Time Series versus Reconstructed Time series.")
plt.xlabel("Number of Records in Time")
