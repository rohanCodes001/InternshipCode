...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 11 13:14:55 2022
...
num = 18   # This number controls which well concentration time series data to use
...
df = ssa.components_to_df(n = 22)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-21-Concentration.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 22) and (22-40).
ssa.calc_wcorr()
ssa.plot_wcorr()
...
# %%
# Looking at First 22 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,22)).plot()
ssa.reconstruct(slice(22,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 22 Components", "Remaining Components", "Original TS"]);
...
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-21-CTMass.joblib', compress=4)
...
df = ssa.components_to_df(n = 35)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-21-CTAqMass.joblib', compress=4)
