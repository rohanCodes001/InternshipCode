...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  1 15:37:56 2022
...
num = 17   # This number controls which well concentration time series data to use
...
df = ssa.components_to_df(n = 31)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W11-92-Concentration.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 31) and (31-40).
ssa.calc_wcorr()
ssa.plot_wcorr()
...
# %%
# Looking at First 31 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,31)).plot()
ssa.reconstruct(slice(31,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 31 Components", "Remaining Components", "Original TS"]);
...
ssa.components_to_df(n = 25)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W11-92-CTMass.joblib', compress=4)
...
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W11-92-CTAqMass.joblib', compress=4)
