...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul  7 11:43:01 2022
...
num = 31   # This number controls which well concentration time series data to use
...
joblib.dump(df, '299-W14-20-Concentration.joblib', compress=4)
...
df = ssa.components_to_df(n = 21)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-20-CTMass.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 21) and (21-40).
ssa.plot_wcorr()
...
# %% Looking at First 21 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,21)).plot()
ssa.reconstruct(slice(21,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 21 Components", "Remaining Components", "Original TS"]);
plt.title("Original Time Series versus Reconstructed Time series.")
plt.xlabel("Number of Records in Time")
...
df = ssa.components_to_df(n = 27)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-20-CTAqMass.joblib', compress=4)
...
# %% Looking at First 27 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,27)).plot()
ssa.reconstruct(slice(27,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 27 Components", "Remaining Components", "Original TS"]);
plt.title("Original Time Series versus Reconstructed Time series.")
plt.xlabel("Number of Records in Time")
