...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul  7 09:43:00 2022
...
num = 29   # This number controls which well concentration time series data to use
...
df = ssa.components_to_df(n = 29)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-74-Concentration.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 29) and (29-40).
ssa.calc_wcorr()
ssa.plot_wcorr()
...
# %%
# Looking at First 29 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,29)).plot()
ssa.reconstruct(slice(29,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 29 Components", "Remaining Components", "Original TS"]);
...
df = ssa.components_to_df(n = 30)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-74-CTMass.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 30) and (30-40).
ssa.plot_wcorr()
...
# %% Looking at First 30 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,30)).plot()
ssa.reconstruct(slice(30,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
...
df = ssa.components_to_df(n = 28)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-74-CTAqMass.joblib', compress=4)
...
# %% Looking at First 28 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,28)).plot()
ssa.reconstruct(slice(28,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 28 Components", "Remaining Components", "Original TS"]);
plt.title("Original Time Series versus Reconstructed Time series.")
plt.xlabel("Number of Records in Time")
