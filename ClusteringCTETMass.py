...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 14 16:49:21 2022
...
well_one = joblib.load('299-W5-1-CTMass.joblib')            # Loading Data
well_two = joblib.load('299-W6-15-CTMass.joblib')
well_three = joblib.load('299-W11-49-CTMass.joblib')
well_four = joblib.load('299-W11-50-CTMass.joblib')
well_five = joblib.load('299-W11-90-CTMass.joblib')
well_six = joblib.load('299-W11-92-CTMass.joblib')
well_seven = joblib.load('299-W11-96-CTMass.joblib')
well_eight = joblib.load('299-W11-97-CTMass.joblib')
well_nine = joblib.load('299-W12-2-CTMass.joblib')
well_ten = joblib.load('299-W12-3-CTMass.joblib')
well_eleven = joblib.load('299-W12-4-CTMass.joblib')  
well_twelve = joblib.load('299-W14-20-CTMass.joblib')
well_thirteen = joblib.load('299-W14-21-CTMass.joblib')
well_fourteen = joblib.load('299-W14-22-CTMass.joblib')
well_fifteen = joblib.load('299-W14-73-CTMass.joblib')
well_sixteen = joblib.load('299-W14-74-CTMass.joblib')
well_seventeen = joblib.load('299-W15-225-CTMass.joblib')
well_eighteen = joblib.load('299-W17-2-CTMass.joblib')
well_nineteen = joblib.load('299-W17-3-CTMass.joblib')
...
fig.suptitle('Time Series Components for CTET mass', fontsize = 35)
for i in range(6):
    for j in range(4):
...
fig.suptitle('Clusters for Time Series Components for CTET Mass', fontsize = 35)
row_i=0
column_j=0
...
for i in range(0,len(namesofmyData)):
    for j in range(0,len(well_name)):
        if namesofmyData[i].replace('-CTMass', '') == well_name[j]:
...
for i,txt in enumerate(namesofmyData):
    ax.annotate(namesofmyData[i].replace('-CTMass', ''), xy = (X[i], Y[i]))    

    
# Spatially Plotting Cluster locations.  
combined_locations = np.vstack((X,Y)).T
fig,ax = plt.subplots(figsize = (12,8))
scatter = ax.scatter(combined_locations[:,0], combined_locations[:,1], c = km.labels_)
ax.set_title("Well Locations labeled with clusters using CTET Mass data")
ax.set_xlabel("X Coordinates")
ax.set_ylabel("Y Coordinates")
for i,txt in enumerate(namesofmyData):
    ax.annotate(namesofmyData[i].replace('-CTMass', ''), xy = (X[i], Y[i]))
...
ax.legend()    
joblib.dump(combined_locations, 'ClusteringCTMass_Locations.joblib', compress=4)
joblib.dump(labels, 'ClusterCTMass_Labels.joblib', compress=4)
joblib.dump(namesofmyData, 'CTMasswell_names.joblib', compress=4)
