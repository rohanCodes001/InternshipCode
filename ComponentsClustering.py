...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 09:11:52 2022
...
well_one = joblib.load('299-W5-1-Concentration.joblib')            # Loading Data
well_two = joblib.load('299-W6-15-Concentration.joblib')
well_three = joblib.load('299-W11-49-Concentration.joblib')
well_four = joblib.load('299-W11-50-Concentration.joblib')
well_five = joblib.load('299-W11-90-Concentration.joblib')
well_six = joblib.load('299-W11-92-Concentration.joblib')
well_seven = joblib.load('299-W11-96-Concentration.joblib')
well_eight = joblib.load('299-W11-97-Concentration.joblib')
well_nine = joblib.load('299-W12-2-Concentration.joblib')
well_ten = joblib.load('299-W12-3-Concentration.joblib')
well_eleven = joblib.load('299-W12-4-Concentration.joblib')  
well_twelve = joblib.load('299-W14-20-Concentration.joblib')
well_thirteen = joblib.load('299-W14-21-Concentration.joblib')
well_fourteen = joblib.load('299-W14-22-Concentration.joblib')
well_fifteen = joblib.load('299-W14-73-Concentration.joblib')
well_sixteen = joblib.load('299-W14-74-Concentration.joblib')
well_seventeen = joblib.load('299-W15-225-Concentration.joblib')
well_eighteen = joblib.load('299-W17-2-Concentration.joblib')
well_nineteen = joblib.load('299-W17-3-Concentration.joblib')
...
test_plume = joblib.load('/Users/shan998/Downloads/rohan_plume/Data/2020_CCl4_plume.joblib')
...
fig.suptitle('Time Series Components for CTET concentrations', fontsize = 35)
for i in range(6):
    for j in range(4):
...
sse = {}
for k in range(1, 10):
    km = TimeSeriesKMeans(n_clusters=k, metric = "dtw", max_iter=1000, random_state=21, dtw_inertia=True).fit(myData)
...
for i in range(0,len(namesofmyData)):
    for j in range(0,len(well_name)):
        if namesofmyData[i].replace('-Concentration', '') == well_name[j]:
...
for i,txt in enumerate(namesofmyData):
    ax.annotate(namesofmyData[i].replace('-Concentration', ''), xy = (X[i], Y[i]))    

    
# Spatially Plotting Cluster locations.  
combined_locations = np.vstack((X,Y)).T
fig,ax = plt.subplots(figsize = (14,10))  # original was 12 by 8 size
scatter = ax.scatter(combined_locations[:,0], combined_locations[:,1], c = km.labels_, cmap = 'viridis')
ax.set_title("Well Locations labeled with clusters using CTET Concentration data")
ax.set_xlabel("X Coordinates")
ax.set_ylabel("Y Coordinates")
for i,txt in enumerate(namesofmyData):
    ax.annotate(namesofmyData[i].replace('-Concentration', ''), xy = (X[i], Y[i]), weight = 'bold')
legend1 = ax.legend(*scatter.legend_elements(),
                    loc="upper left", title="Clusters")
ax.add_artist(legend1)    
ax.legend()
joblib.dump(combined_locations, 'Clustering_Locations.joblib', compress=4)
joblib.dump(labels, 'Cluster_Labels.joblib', compress=4)
joblib.dump(namesofmyData, 'well_names.joblib', compress=4)
...
# The Davies–Bouldin index (DBI) is, 
