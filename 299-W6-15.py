...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 11 10:33:31 2022
...
num = 19   # This number controls which well concentration time series data to use
...
joblib.dump(df, '299-W6-15-Concentration.joblib', compress=4)
...
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W6-15-CTMass.joblib', compress=4)
...
df = ssa.components_to_df(n = 34)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W6-15-CTAqMass.joblib', compress=4)
...
ssa.reconstruct(slice(0,34)).plot()
ssa.reconstruct(slice(34,40)).plot()
ssa.orig_TS.plot(alpha=0.4, color = 'red')
plt.legend(["First 34 Components", "Remaining Components", "Original TS"]);
plt.title("Original Time Series versus Reconstructed Time series.")
plt.xlabel("Dates")
