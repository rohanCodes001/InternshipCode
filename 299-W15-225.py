...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  8 12:54:56 2022
...
num = 32   # This number controls which well concentration time series data to use
...
df = ssa.components_to_df(n = 13)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W15-225-Concentration.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 13) and (13-40).
ssa.calc_wcorr()
ssa.plot_wcorr()
...
# %%
# Looking at First 13 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,13)).plot()
ssa.reconstruct(slice(13,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 13 Components", "Remaining Components", "Original TS"]);
...
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W15-225-CTMass.joblib', compress=4)
...
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W15-225-CTAqMass.joblib', compress=4)
