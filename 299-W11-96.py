...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul  7 10:40:49 2022
...
num = 25   # This number controls which well concentration time series data to use
...
df = ssa.components_to_df(n = 18)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W11-96-Concentration.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 18) and (18-40).
ssa.calc_wcorr()
ssa.plot_wcorr()
...
# %%
# Looking at First 18 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,18)).plot()
ssa.reconstruct(slice(18,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 18 Components", "Remaining Components", "Original TS"]);
...
df = ssa.components_to_df(n = 23)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W11-96-CTMass.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 23) and (23-40).
ssa.plot_wcorr()
...
# %% Looking at First 23 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,23)).plot()
ssa.reconstruct(slice(23,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 23 Components", "Remaining Components", "Original TS"]);
plt.title("Original Time Series versus Reconstructed Time series.")
plt.xlabel("Number of Records in Time")
...
df = ssa.components_to_df(n = 26)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W11-96-CTAqMass.joblib', compress=4)
...
# %% Looking at First 26 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,26)).plot()
ssa.reconstruct(slice(26,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 26 Components", "Remaining Components", "Original TS"]);
plt.title("Original Time Series versus Reconstructed Time series.")
plt.xlabel("Number of Records in Time")
