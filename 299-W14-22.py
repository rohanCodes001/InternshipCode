...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 11 13:32:36 2022
...
num = 13   # This number controls which well concentration time series data to use
...
df = ssa.components_to_df(n = 15)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-22-Concentration.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 15) and (15-40).
ssa.calc_wcorr()
ssa.plot_wcorr()
...
ssa.reconstruct(0).plot()
ssa.orig_TS.plot(alpha=0.4, color = 'black')
plt.title("Concentration Time Series: Trend Component")     # Trend only for presentation.
plt.xlabel("Number of Records in Time")
plt.ylabel("CT Concentrations")
plt.ylabel("CT Concentrations")
legend = ["Trend Component", "Original Time Series"]
plt.legend(legend);
...
# %%
# Looking at First 15 elements and showing the last elements since 40 was our window size.
ssa.reconstruct(slice(0,15)).plot()
ssa.reconstruct(slice(15,40)).plot()
ssa.orig_TS.plot(alpha=0.5, color = 'black')
plt.legend(["First 15 Components", "Remaining Components", "Original TS"]);
...
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-22-CTMass.joblib', compress=4)

# %%
# Here 2 groups seem to be seen (0 to 35) and (35-40).
ssa.plot_wcorr()
...
df = ssa.components_to_df(n = 35)
df.set_index(new_dates, inplace = True)
joblib.dump(df, '299-W14-22-CTAqMass.joblib', compress=4)
